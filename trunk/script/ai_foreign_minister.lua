...
function ForeignMinister_HandlePeace()
	...
	if ForeignMinisterData.HasFaction then
		...
		local auhTag = CCountryDataBase.GetTag("AUH")
		local itaTag = CCountryDataBase.GetTag("ITA")
		

		-- avoid breaking the triple alliance
		for loTargetCountryTag in ForeignMinisterData.ministerCountry:GetAllies() do
			local loAction = CAllianceAction(ForeignMinisterData.ministerTag, loTargetCountryTag)
			loAction:SetValue(false) -- cancel
			--Utils.LUA_DEBUGOUT("break alliance check... Target = "..tostring(loTargetCountryTag).." Initiator = " ..tostring(ForeignMinisterData.ministerTag).." checking for t alliance")
		
			if loTargetCountryTag == itaTag and (ForeignMinisterData.ministerTag == gerTag or ForeignMinisterData.ministerTag == auhTag) then
				--Utils.LUA_DEBUGOUT("Avoided breaking t alliance... Target = "..tostring(loTargetCountryTag).." Initiator = " ..tostring(ForeignMinisterData.ministerTag))
			else
				--Utils.LUA_DEBUGOUT("Breaking alliance... Target = "..tostring(loTargetCountryTag).." Initiator = " ..tostring(ForeignMinisterData.ministerTag))
				if loAction:IsSelectable() then
					ForeignMinisterData.minister:Propose(loAction, 100)
				end
			end
